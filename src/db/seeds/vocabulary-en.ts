...
async function enrichBatch(words: string[]): Promise<z.infer<typeof vocabItemSchema>[]> {
    ...
    messages: [
      {
        role: "system",
        content:
          "You are an English learner dictionary for ESL students. For each English word provided, return a JSON object with an `items` array where each element has: word (exact English word as given), translation (concise English definition or synonym, 1-8 words), example_sentence (one natural English sentence using the word, 8-15 words, appropriate for language learners). Return JSON only.",
      },
      {
        role: "user",
  ...
  for (const item of rawItems) {
    ...
    if (!validated.success) {
      console.warn("[vocabulary-en] Skipping malformed item:", item);
      continue;
    }
    if (!validWords.has(validated.data.word)) {
      console.warn("[vocabulary-en] Filtered hallucinated word:", validated.data.word);
      continue;
    }
...
export async function seedVocabularyEn(): Promise<number> {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    console.log(`[vocabulary-en] Processing batch ${i + 1}/${batches.length} (${batch.length} words)...`);

    try {
      const enriched = await enrichBatch(batchWords);
      if (enriched.length === 0) {
        console.warn(`[vocabulary-en] Batch ${i + 1}: no valid items returned; skipping upsert.`);
      ...
      if (error) {
        console.error(`[vocabulary-en] Batch ${i + 1} upsert failed:`, error.message);
        await sleep(BATCH_DELAY_MS);
        continue;
      }

      batchesSucceeded++;
      console.log(`[vocabulary-en] Batch ${i + 1}: upserted ${rows.length} rows.`);
    } catch (error) {
      console.error(`[vocabulary-en] Batch ${i + 1} failed:`, error);
    }

    await sleep(BATCH_DELAY_MS);
  }

  console.log("[vocabulary-en] Done.");
  if (batches.length > 0 && batchesSucceeded === 0) {
    console.error("[vocabulary-en] No batches completed successfully.");
    return 1;
  }
  return 0;
}
