// Source: OpenSubtitles frequency list (CC BY 4.0)
...
function readWordsFromTxt(filePath: string, limit: number): string[] {
...
async function enrichBatch(words: string[]): Promise<z.infer<typeof vocabItemSchema>[]> {
  ...
  for (const item of rawItems) {
    ...
    if (!validated.success) {
      console.warn("[vocabulary] Skipping malformed item:", item);
      continue;
    }
    if (!validWords.has(validated.data.word)) {
      console.warn("[vocabulary] Filtered hallucinated word:", validated.data.word);
      continue;
    }
...
export async function seedVocabulary(): Promise<void> {
  const rawWords = readWordsFromTxt(TXT_PATH, WORD_LIMIT);
  const words = rawWords.map(sanitizeWordForPrompt).filter((w) => w.length > 0);
  if (words.length === 0) {
    console.log("[vocabulary] No words found in text file.");
    return;
  }

  const wordsWithRank: WordWithRank[] = words.map((word, index) => ({
    word,
  ...
  console.log(`[vocabulary] Loaded ${wordsWithRank.length} words, ${batches.length} batches.`);
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    console.log(`[vocabulary] Processing batch ${i + 1}/${batches.length} (${batch.length} words)...`);

    try {
      const enriched = await enrichBatch(batchWords);
      if (enriched.length === 0) {
        console.warn(`[vocabulary] Batch ${i + 1}: no valid items returned; skipping upsert.`);
      ...
      batchesSucceeded++;
      console.log(`[vocabulary] Batch ${i + 1}: upserted ${rows.length} rows.`);
    } catch (error) {
      console.error(`[vocabulary] Batch ${i + 1} failed:`, error);
    }
  ...
  if (batches.length > 0 && batchesSucceeded === 0) {
    throw new Error("[vocabulary] No batches completed successfully.");
  }
}
