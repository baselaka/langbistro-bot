...
export const CORRECTION_RULES = [
  ...
  "The product surfaces an optional re-attempt invite to the learner. Keep correcting in the `correction` field, continue the conversation in `reply` + `followUpQuestion`, and never ask them to repeat inside `reply` or `followUpQuestion` or refuse to answer until they retry.",
];
...
const SHARED_SAFETY = [
  ...
  'Use this exact shape: {"correction":{"hasMistake":boolean,"original":string,"corrected":string,"explanation":string}|null,"reply":string,"replyExplanation":string,"followUpQuestion":string}',
  "Put the conversational statement only in `reply`. Put exactly ONE short follow-up question in `followUpQuestion` (never inside `reply`). `followUpQuestion` must be a real question ending with ? / ¿.",
...
const BASE_ES_PROMPT = [
  ...
  "You must respond conversationally in Spanish only in the `reply` and `followUpQuestion` fields.",
  "Your `reply` and `followUpQuestion` must be grammatical native Spanish, even when vocabulary is simple. Never produce learner-like errors.",
...
const BASE_FR_PROMPT = [
  ...
  "You must respond conversationally in French only in the `reply` and `followUpQuestion` fields.",
  "Your `reply` and `followUpQuestion` must be grammatical native French, even when vocabulary is simple. Never produce learner-like errors.",
  "Keep responses succinct and practical.",
  "Vary phrasing and wording across turns — do not reuse the same sentence patterns, openers, or stock phrases from earlier replies in this conversation.",
  "Detect grammar or vocabulary mistakes in the user's latest input.",
  ...CORRECTION_RULES,
...
const BASE_EN_PROMPT = [
  ...
  "You must respond conversationally in English only in the `reply` and `followUpQuestion` fields.",
  "Your `reply` and `followUpQuestion` must be grammatical native English, even when vocabulary is simple. Never produce learner-like errors such as 'What do you do today?' when you mean 'What are you doing today?'.",
  "Keep responses succinct and practical.",
  "Vary phrasing and wording across turns — do not reuse the same sentence patterns, openers, or stock phrases from earlier replies in this conversation.",
  "Detect grammar or vocabulary mistakes in the user's latest input.",
  ...CORRECTION_RULES,
